...
def propagate_integration_error(cb_func, params, param_errors, delta=1e-4):
    """
    Estimate the propagated uncertainty in the integral using finite difference approximation.
    """
    base_integral = integrate_cb2d_2sigma_root(cb_func, params)
    squared_error = 0.0

    for i, (p_val, p_err) in enumerate(zip(params, param_errors)):
        if p_err == 0:
            continue
        ...
        int_up = integrate_cb2d_2sigma_root(cb_func, params_up)
        int_down = integrate_cb2d_2sigma_root(cb_func, params_down)

        partial_derivative = (int_up - int_down) / (2 * delta)
        squared_error += (partial_derivative * p_err) ** 2
...
with open(results_file_path, "w") as f:
    f.write("# angle mu_x mu_x_err sigma_x sigma_x_err mu_y mu_y_err sigma_y sigma_y_err "
    ...
    for angle_index, angle in enumerate(angles):
        ...
        integrated_cb_raw_err = propagate_integration_error(cb2d_integration_ready, fitted_params, errors)
